...
完整体包含 Docker 依赖、FastAPI 后端和 Celery worker：

```bash
cd /home/chenjy/桌面/RAGENTv2
bash scripts/start_ragent_full.sh
```

停止 API 和 Celery：

```bash
bash scripts/stop_ragent_full.sh
```

同时停止 MySQL、Redis、RabbitMQ 容器：

```bash
bash scripts/stop_ragent_full.sh --all
```

`docker compose down` 只停止并删除容器，默认保留数据卷；不要加 `-v`，除非你明确要删除 MySQL/Redis/RabbitMQ 数据。
